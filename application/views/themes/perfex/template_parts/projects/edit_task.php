...
<div class="row">
    ...
    echo 'selected';
                        ...
                        <option value="3" <?php if ($task->priority == 3) {
    echo 'selected';
} ?>><?php echo _l('task_priority_high'); ?></option>
                        <option value="4" <?php if ($task->priority == 4) {
    echo 'selected';
    ...
    echo form_hidden('milestone', $task->milestone);
        ...
        <div class="row">
            <div class="col-md-6">
                <?php echo render_date_input('startdate', 'task_add_edit_start_date', _d($task->startdate), ['required' => true]); ?>
            </div>
            <div class="col-md-6">
                <?php echo render_date_input('duedate', 'task_add_edit_due_date', $task->duedate, $project->deadline ? ['data-date-end-date' => $project->deadline] : []); ?>
            </div>
        </div>
    ...
    echo ' selected';
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="description" rows="10"
                class="form-control"><?php echo clear_textarea_breaks($task->description); ?></textarea>
        </div>
        <?php echo render_custom_fields('tasks', $task->id, ['show_on_client_portal' => 1]); ?>

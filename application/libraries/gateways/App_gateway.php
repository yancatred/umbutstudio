...
class App_gateway
{
    ...
    public function setSettings($settings)
    {
        ...
        if ($this->processingFees) {
            array_push($requiredSettings,
                [
                    'name'          => 'fee_fixed',
                    'default_value' => 0,
                    'label'         => 'payment_gateway_fee_fixed',
                ],
                [
                    'name'          => 'fee_percent',
                    'default_value' => 0,
                    'label'         => 'payment_gateway_fee_percentage',
                ]
            );
        }
     ...
     * amount - Required
     * invoiceid - Required
     * payment_attempt_reference - Required if processingFees enabled
    ...
    public function addPayment($data)
    {
        ...
        if ($this->processingFees && $data['payment_attempt_reference']) {
            $this->ci->load->model('payment_attempts_model');
            $attempt = $this->ci->payment_attempts_model->getByReference($data['payment_attempt_reference'], $this->getId());
            if ($attempt) {
                $data['amount'] -= $attempt->fee;
            }
        }

        unset($data['payment_attempt_reference']);
        $this->ci->load->model('payments_model');
        if ($this->ci->payments_model->add($data)) {
            if (isset($attempt)) {
                $this->ci->payment_attempts_model->delete($attempt->id);
            }
            return true;
        }
        return false;
    ...
    {
        if ($this->processingFees) {
            return round($this->getPercentageFee($amount) + $this->getFixedFee(), get_decimal_places());
        }
        return 0;
    }
    ...
    {
        return (float) $this->getSetting('fee_fixed');
    }

    public function getPercentageFee(float $amount): float
    {
        $fee = (float) $this->getSetting('fee_percent');

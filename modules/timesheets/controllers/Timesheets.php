...
class timesheets extends AdminController
{
	...
	public function add_requisition_ajax()
	{
		...
		if ($this->input->post()) {
			...
			if ($data['rel_type'] == '1') {
				...
				$rel_type = $this->timesheets_model->get_leave_rel_type_mapping($data['type_of_leave']);
			...
			if ($result != '') {
				...
				if ($check_proccess) {
					if ($check_proccess->choose_when_approving == 0) {
						$result_approve = $this->timesheets_model->send_request_approve($data_app, $data['staff_id']);
						
						// Note: send_request_approve returns TRUE on success, string (approver type) on failure
						$is_success = ($result_approve === true);
	...
	public function requisition_detail($id)
	{
		...
		if ($data['request_leave']->rel_type == 1) {
			// Use dynamic mapping instead of hardcoded switch statement
			$rel_type = $this->timesheets_model->get_leave_rel_type_mapping($data['request_leave']->type_of_leave);
	...
	public function set_leave()
	{
		if ($this->input->post()) {
			...
			// Cast type_of_leave to integer to ensure it's stored as ID, not string
			if (isset($data['type_of_leave'])) {
				$data['type_of_leave'] = (int)$data['type_of_leave'];
			}
	...
	/**
	 * get leave balance ajax
	 * @return json
	 */
	public function get_leave_balance_ajax()
	{
		if ($this->input->post()) {
			$staff_id = $this->input->post('staff_id') ?: get_staff_user_id();
			$type_of_leave = $this->input->post('type_of_leave') ?: '';
			
			$day_off = $this->timesheets_model->get_current_date_off($staff_id, $type_of_leave);

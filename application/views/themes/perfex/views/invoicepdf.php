...
// Adjust the Y position for to avoid header
//$pdf->ln(40);
...
$info_right_column .= '<b style="color:#4e4e4e;">' . _l('invoice_no') . ' ' . $invoice_number . '</b>';
...
// Initialize $invoice_info variable
...
// Initialize $date_info variable for date information
$date_info = '<br /><div >';
$date_info .= _l('invoice_data_date') . ' ' . _d($invoice->date) . '<br />';

// Include due date if available
if (!empty($invoice->duedate)) {
    $date_info .= _l('invoice_data_duedate') . ' ' . _d($invoice->duedate) . '<br />';
}

// Include sale agent if applicable
if ($invoice->sale_agent && get_option('show_sale_agent_on_invoices') == 1) {
    $date_info .= _l('sale_agent_string') . ': ' . get_staff_full_name($invoice->sale_agent) . '<br />';
}

// Include project name if applicable
if ($invoice->project_id && get_option('show_project_on_invoice') == 1) {
    $date_info .= _l('project') . ': ' . get_project_name_by_id($invoice->project_id) . '<br />';
}
...
// Apply hooks to allow additional modifications
$date_info = hooks()->apply_filters('invoice_pdf_header_after_date', $date_info, $invoice);
$date_info = hooks()->apply_filters('invoice_pdf_header_after_due_date', $date_info, $invoice);
$date_info = hooks()->apply_filters('invoice_pdf_header_after_sale_agent', $date_info, $invoice);
$date_info = hooks()->apply_filters('invoice_pdf_header_after_project_name', $date_info, $invoice);
$date_info = hooks()->apply_filters('invoice_pdf_header_before_custom_fields', $date_info, $invoice);
$date_info = hooks()->apply_filters('invoice_pdf_header_after_custom_fields', $date_info, $invoice);

//untuk funtion swap
$swap = get_option('swap_pdf_info');

// Assign $left_info and $right_info based on $swap
$left_info = $swap == '1' ? $date_info : $invoice_info;
$right_info = $swap == '1' ? $invoice_info : $date_info;
...
<tr >
    ...
    <td align="right" width="15%">' . app_format_money($invoice->subtotal, true) . '</td>
</tr>';
...
<tr style="background-color:#f0f0f0;">
    ...
    <td align="right" width="15%">' . app_format_money($invoice->total, true) . '</td>
</tr>';
...
if (get_option('show_credits_applied_on_invoice') == 1 && $credits_applied = total_credits_applied_to_invoice($invoice->id)) {
    ...
    <tr>
        ...
        <td align="right" width="15%">-' . app_format_money($credits_applied, true) . '</td>
    </tr>';
}
...
if (get_option('show_amount_due_on_invoice') == 1 && $invoice->status != Invoices_model::STATUS_CANCELLED) {
    $tbltotal .= '<tr style="background-color:#f0f0f0;">
       ...
       <td align="right" width="15%">' . app_format_money($invoice->total_left_to_pay, true) . '</td>
   </tr>';
}

...
class timesheets_model extends app_model
{
	...
	public function get_day_off($staffid = 0, $year = 0, $type_of_leave = '')
	{
		...
		$day_off = $this->db->query('select * from ' . db_prefix() . 'timesheets_day_off where staffid = ' . $staffid . ' and year = ' . $year . ' and type_of_leave = ' . (int)$type_of_leave)->row();
		return $day_off;
	}
	/**
	...
	public function send_request_approve($data, $staff_id = '')
	{
		...
		foreach ($data_new as $index => $value) {
			...
			if ($value->approver !== 'specific_personnel') {
				...
				// DEBUG: Log approve_value result

				if (is_numeric($approve_value) && $approve_value > 0) {
					$staff_email = $this->staff_model->get($approve_value)->email;
					$approve_value = $staff_email;
				} else {
					// CHANGED: Skip this approver instead of failing entire process
					continue; // Skip to next approval level
				}
				$row['approve_value'] = $approve_value;

				$staffid = $this->get_staff_id_by_approve_value($value, $value->approver);
				
				// DEBUG: Log final staffid
				...
				// DEBUG: Log the row being inserted
				...
				if (empty($value->staff)) {
					continue; // Skip if no specific staff assigned
				}
				...
				// DEBUG: Log specific personnel insertion
	...
	public function get_approve_setting($type, $only_setting = true, $staff_id = '')
	{
		...
		// DEBUG: Log staff and department info
		...
		if ($where_job_position != '' && $where_departments != '') {
			$query_condition = $where_job_position . ' AND ' . $where_departments . ' AND related="' . $type . '"';
		...
		if ($approval_setting) {
			...
			// DEBUG: Try fallback queries
			
			$this->db->select('*');
			$this->db->where('related', $type);
			if ($where_job_position != '') {
				$fallback_condition = $where_job_position . ' AND (departments is null OR departments = "") AND related="' . $type . '"';
				$this->db->where($fallback_condition);
			}
			$approval_setting = $this->db->get(db_prefix() . 'timesheets_approval_setting')->row();
			...
			if ($approval_setting) {
				...
				// Final fallback - check if ANY record exists with this related type
				$this->db->select('*');
				$this->db->where('related', $type);
				$any_setting = $this->db->get(db_prefix() . 'timesheets_approval_setting')->row();
	...
	public function add_requisition_ajax($data)
	{
		...
		$type = 'leave';
		if ($data['rel_type'] == 1) {
			// Use dynamic mapping instead of hardcoded 'leave'
			$type = $this->get_leave_rel_type_mapping($data['type_of_leave']);
	...
	/**
	 * Get the correct related approval type for a leave type ID
	 * Maps leave types to their approval setting "related" values
	 * @param  int $type_of_leave_id
	 * @return string
	 */
	/**
	 * Get leave rel type mapping dynamically from database
	 * Uses intelligent matching to map leave types to approval settings
	 * @param  int $type_of_leave_id
	 * @return string
	 */
	public function get_leave_rel_type_mapping($type_of_leave_id)
	{
		$custom_type = $this->get_type_of_leave($type_of_leave_id);
		if ($custom_type && isset($custom_type->type_name)) {
			$leave_name = strtoupper(trim($custom_type->type_name));
			
			// DATABASE-DRIVEN MAPPING: Get all approval settings and find best match
			$this->db->select('name, related');
			$approval_settings = $this->db->get(db_prefix() . 'timesheets_approval_setting')->result();
			...
			foreach ($approval_settings as $setting) {
				$setting_name = strtoupper(trim($setting->name));
				$current_score = 0;
				
				// Strategy 1: Exact match (highest priority)
				if ($setting_name === $leave_name) {
					return $setting->related;
				}
				
				// Strategy 2: Contains key words (SICK/MEDICAL, ANNUAL/YEARLY, etc.)
				$leave_keywords = explode(' ', $leave_name);
				$setting_keywords = explode(' ', $setting_name);
				
				foreach ($leave_keywords as $leave_word) {
					if (strlen($leave_word) >= 4) { // Skip short words like "AM", "PM"
						foreach ($setting_keywords as $setting_word) {
							if ($leave_word === $setting_word) {
								$current_score += 10;
							} elseif (strpos($leave_word, $setting_word) !== false || strpos($setting_word, $leave_word) !== false) {
								$current_score += 5;
							}
						}
					}
				...
				// Strategy 3: Special semantic matching for common variations
				$semantic_matches = [
					'SICK' => ['MEDICAL', 'MC'],
					'MEDICAL' => ['SICK', 'MC'],
					'ANNUAL' => ['YEARLY', 'VACATION'],
					'YEARLY' => ['ANNUAL', 'VACATION'],
					'BIRTHDAY' => ['BIRTH'],
					'MATERNITY' => ['MATERNAL', 'PREGNANCY'],
					'PATERNITY' => ['PATERNAL', 'FATHER'],
					'COMPASSIONATE' => ['BEREAVEMENT', 'EMERGENCY'],
					'UNPAID' => ['WITHOUT'],
					'REPLACEMENT' => ['COMPENSATORY', 'COMP']
				];
				
				foreach ($leave_keywords as $leave_word) {
					if (isset($semantic_matches[$leave_word])) {
						foreach ($semantic_matches[$leave_word] as $synonym) {
							if (strpos($setting_name, $synonym) !== false) {
								$current_score += 15;
							}
						}
					}
			...
			if ($best_match && $match_score >= 10) {
				return $best_match->related;
			}
			
			// Fallback: Return original type name for admin to configure
			return $custom_type->type_name;
		}
		
		// Legacy mappings ONLY for old hardcoded IDs (minimal backward compatibility)
		$legacy_mappings = [
			8 => 'Leave',
			2 => 'maternity_leave', 
			1 => 'sick_leave',
		...
		if (isset($legacy_mappings[$type_of_leave_id])) {
			return $legacy_mappings[$type_of_leave_id];
		}
		
		return (string)$type_of_leave_id . ' (Unknown Leave Type)';
	}
	/**

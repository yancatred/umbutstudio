<script>
  ...
  (function(){
    ...
    $('#rel_type').on('change', function() {
      ...
      if(value == 1)
      {
        ...
        // Show remaining days for ALL leave types (removed hardcoded ID 8 restriction)
    ...
    function get_day_from_date(){
      ...
      $.post(admin_url + 'timesheets/calculate_number_days_off',data).done(function(response){
       ...
       // Validate remaining days for ALL leave types (removed hardcoded ID 8 restriction)
  ...
  function get_remain_day_off(){
    "use strict";
    var staff_id = $('select[name="staff_id"]').val() || $('input[name="userid"]').val();
    var type_of_leave = $('select[name="type_of_leave"]').val();

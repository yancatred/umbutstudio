...
// FULLY DYNAMIC mapping - build leave type mapping from database ONLY
$leave_types = $this->ci->timesheets_model->get_type_of_leave();
$leave_mapping_parts = [];

// Add ALL leave types from database dynamically - NO HARDCODED VALUES
foreach ($leave_types as $type) {
    $leave_mapping_parts[] = 'IF(' . db_prefix() . 'timesheets_requisition_leave.type_of_leave = ' . $type['id'] . ',"' . $type['type_name'] . '"';
}

// Add fallback for type_of_leave_text field (for backward compatibility)
$leave_mapping_parts[] = 'IF(' . db_prefix() . 'timesheets_requisition_leave.type_of_leave = 0,' . db_prefix() . 'timesheets_requisition_leave.type_of_leave_text,"")';

// Build nested IF statement
$add_query = ' ' . implode(',', $leave_mapping_parts) . str_repeat(')', count($leave_mapping_parts));
...
foreach ($rResult as $aRow) {
	...
	if ($aRow['rel_type'] == 1) {
		...
		// Use dynamic mapping instead of hardcoded switch statement

		$rel_type = $this->ci->timesheets_model->get_leave_rel_type_mapping($aRow['type_of_leave']);
	...
	if ($rel_type != '') {
		...
		// Fallback - try to get name from type_of_leave_text field
		...
		if ($rel_type_name) {
			...
			// If still empty, try using type_of_leave_text directly

...
// Initialize $credit_note_info variable with "Bill to" information
...
// Include shipping information if applicable
...
// Apply hooks after shipping info
$credit_note_info = hooks()->apply_filters('credit_note_pdf_header_after_shipping_info', $credit_note_info, $credit_note);

// Initialize $date_info variable for date-related information with an empty space on top
$date_info = '<br /><div style="color:#424242;">';
$date_info .= _l('credit_note_date') . ': ' . _d($credit_note->date) . '<br />';

// Apply hooks after date
$date_info = hooks()->apply_filters('credit_note_pdf_header_after_date', $date_info, $credit_note);

// Include reference number if available
if (!empty($credit_note->reference_no)) {
    $date_info .= _l('reference_no') . ': ' . $credit_note->reference_no . '<br />';
    $date_info = hooks()->apply_filters('credit_note_pdf_header_after_reference_no', $date_info, $credit_note);
}

// Include project name if applicable
if ($credit_note->project_id && get_option('show_project_on_credit_note') == 1) {
    $date_info .= _l('project') . ': ' . get_project_name_by_id($credit_note->project_id) . '<br />';
    $date_info = hooks()->apply_filters('credit_note_pdf_header_after_project', $date_info, $credit_note);
}

// Add custom fields
$date_info = hooks()->apply_filters('credit_note_pdf_header_before_custom_fields', $date_info, $credit_note);
foreach ($pdf_custom_fields as $field) {
    $value = get_custom_field_value($credit_note->id, $field['id'], 'credit_note');
    if ($value != '') {
        $date_info .= $field['name'] . ': ' . $value . '<br />';
    }
}
$date_info .= '</div>';
$date_info = hooks()->apply_filters('credit_note_pdf_header_after_custom_fields', $date_info, $credit_note);

//untuk funtion swap
$swap = get_option('swap_pdf_info');

// Assign $left_info and $right_info based on $swap
$left_info = $swap == '1' ? $date_info : $credit_note_info;
$right_info = $swap == '1' ? $credit_note_info : $date_info;
...
if (get_option('total_to_words_enabled') == 1) {
    ...
    $pdf->writeHTMLCell('', '', '', '', _l('num_word') . ': ' . $CI->numberword->convert($credit_note->total, $credit_note->currency_name), 0, 1, false, true, 'C', true);

...
<script>
	...
	function uploadfilecsv(event){
		...
		if(($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')){
			...
			$.ajax({ 
				url: admin_url + 'hr_payroll/import_commissions_excel', 
				method: 'post', 
				data: formData, 
				contentType: false, 
				processData: false
	...
	function dowload_commissions_excel(){
		'use strict';

		var formData = new FormData();
		formData.append("csrf_token_name", $('input[name="csrf_token_name"]').val());
		formData.append("month_commissions", $('input[name="month_commissions"]').val());
		$.ajax({ 
			url: admin_url + 'hr_payroll/create_commissions_sample_file', 
			method: 'post', 
			data: formData, 
			contentType: false, 
			processData: false
			...
			if(response.success == true){
				...
				alert_float('success', "<?php echo _l("create_commissions_file_success") ?>");
				...
				alert_float('warning', "<?php echo _l("create_commissions_file_false") ?>");
			}
		});
	}

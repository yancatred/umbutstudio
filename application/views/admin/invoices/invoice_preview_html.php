            ...
            <table class="table text-right">
                <tbody>
                    <tr id="subtotal">
                        ...
                        <td class="subtotal">
                            <?php echo app_format_money($invoice->subtotal, 2); ?>
                        </td>
                    </tr>
                    <?php if (is_sale_discount_applied($invoice)) { ?>
                        <tr>
                            ...
                            <td class="discount">
                                <?php echo '-' . app_format_money($invoice->discount_total, 2); ?>
                            </td>
                        </tr>
                    <?php } ?>
                    <?php
                    foreach ($items->taxes() as $tax) {
                        echo '<tr class="tax-area"><td class="bold !tw-text-neutral-700">' . $tax['taxname'] . ' (' . app_format_number($tax['taxrate']) . '%)</td><td>' . app_format_money($tax['total_tax'], 2) . '</td></tr>';
                    }
                    ?>
                    <?php if ((int)$invoice->adjustment != 0) { ?>
                        <tr>
                            ...
                            <td class="adjustment">
                                <?php echo app_format_money($invoice->adjustment, 2); ?>
                            </td>
                        </tr>
                    <?php } ?>
                    <tr>
                        ...
                        <td class="total">
                            <?php echo app_format_money($invoice->total, 2); ?>
                        </td>
                    </tr>
                    <?php if (count($invoice->payments) > 0 && get_option('show_total_paid_on_invoice') == 1) { ?>
                        <tr>
                            ...
                            <td>
                                <?php echo '-' . app_format_money(sum_from_table(db_prefix() . 'invoicepaymentrecords', ['field' => 'amount', 'where' => ['invoiceid' => $invoice->id]]), 2); ?>
                            </td>
                        </tr>
                    <?php } ?>
                    <?php if (get_option('show_credits_applied_on_invoice') == 1 && $credits_applied = total_credits_applied_to_invoice($invoice->id)) { ?>
                        <tr>
                            ...
                            <td>
                                <?php echo '-' . app_format_money($credits_applied, 2); ?>
                            </td>
                        </tr>
                    <?php } ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="<?php echo $invoice->total_left_to_pay > 0 ? 'text-danger ' : ''; ?>">
                                    <?php echo app_format_money($invoice->total_left_to_pay, 2); ?>
                                </span>
                            </td>
                        </tr>
                    <?php } ?>

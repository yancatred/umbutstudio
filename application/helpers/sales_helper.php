...
if (!function_exists('format_organization_info')) {
     ...
     * @return string
     */
    function format_organization_info($return_only_company_name = false)
    {
        $format = get_option('company_info_format');
        $vat    = get_option('company_vat');

        $company_name = '<b style="color:black" class="company-name-formatted">' . get_option('invoice_company_name') . '</b>';

        if ($return_only_company_name) {
            // If true, return only the company name
            return $company_name;
        } else {
            // Otherwise, return everything except the company name
        ...
        $format = _info_format_replace('phone_number_with_label', get_option('invoice_company_phonenumber'), $format); // Add this line to include phone number with label
        $format = _info_format_replace('vat_number', $vat, $format);
...
function sales_number_format($number, $format, $applied_prefix, $date)
{
    ...
    if ($format == 1) {
        ...
        $number = $applied_prefix . date('m', strtotime($date)) . '/' . date('Y', strtotime($date)) . '/' . str_pad($number, $prefixPadding, '0', STR_PAD_LEFT);
    }

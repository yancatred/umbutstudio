...
class Hr_payroll_model extends App_Model {
	 ...
	 * @return [type]       
	 */
	public function updateIncomeTaxRates(array $data): bool
{
    $affectedRows = 0;

    $incomeTaxRateHs = $data['incometax_rate_hs']?? null;
    unset($data['incometax_rate_hs']);

    if ($incomeTaxRateHs!== null) {
        $incomeTaxRateDetails = json_decode($incomeTaxRateHs, true);
        ...
        $taxRateDetails = [];
        foreach ($incomeTaxRateDetails as $key => $value) {
            if ($value[2]!== '') {
                $taxRateDetails[] = array_combine($taxRateHeaders, $value);
            }
        }
        ...
        foreach ($taxRateDetails as $key => $value) {
            if ($value['id']!== '') {
                $dbOperations['delete'][] = $value['id'];
                $dbOperations['update'][] = $value;
            } else {
                unset($value['id']);
                $dbOperations['insert'][] = $value;
            }
        }

        if (empty($dbOperations['delete'])) {
            $dbOperations['delete'] = ['0'];
        }
        $dbOperations['delete'] = implode(',', $dbOperations['delete']);

        try {
            $this->db->trans_start();

            $this->db->where('id NOT IN ('. $dbOperations['delete']. ')');
            $this->db->delete(db_prefix(). 'hrp_income_tax_rates');
            if ($this->db->affected_rows() > 0) {
                $affectedRows++;
            }

            if (count($dbOperations['insert']) > 0) {
                $affectedRows += $this->db->insert_batch(db_prefix(). 'hrp_income_tax_rates', $dbOperations['insert']);
            }

            if (count($dbOperations['update']) > 0) {
                $affectedRows += $this->db->update_batch(db_prefix(). 'hrp_income_tax_rates', $dbOperations['update'], 'id');
            }
            ...
            log_message('error', 'Error updating income tax rates: '. $e->getMessage());
            return false;
        }
    }
...
/*TO DO: XỬ LÝ CẬP NHẬT, cập nhật thông tin nhân viên sau khi người dùng thực hiện thay đổi trên handsome table, dùng cho 2 trường hợp: có tích hợp HR records module, không tích hợp Hr records module.  đang xử lý!!!*/
	...
	{	
		...
		foreach ($payslip_detail as $key => $value) {
			...
			// Commented out the lines removing bank_name and account_number
    ...
    public function remove_employees_not_under_management_on_payslip($payslip_data) {
        ...
        if (is_array($payslip_data_decode)) {
            if (isset($payslip_data_decode[0]->celldata)) {
                ...
                // Get rows to remove from payslip
    ...
    /**
     * Get table staff data
     * @param  int $staffid 
     * @return array|false             
     */
    public function get_table_staff_data($staffid) {
        try {
            // Prepare the SQL statement to fetch data from either 'staffid' or 'staff_id'
            $stmt = $this->pdo->prepare("SELECT * FROM tblstaff WHERE staffid = :staffid OR staff_id = :staffid");
	 ...
	 // this is model
	//  public function get_timesheets_data()
	//  {
	// 	 $this->db->select('staffid, year, type_of_leave');
	// 	 $this->db->from(db_prefix() . 'tbltimesheets_day_off');
	// 	 $query = $this->db->get();

...
class hr_payroll extends AdminController {
	...
	public function employee_export_pdf($id) {
		...
		// Extract year from payslip month
		$month_date = $data['payslip_detail']['month'];
		$year = date('Y', strtotime($month_date));
	
		// Fetch employee details for the same year
		$employee = $this->hr_payroll_model->get_employees_data($year, '', ' staff_id = ' . $data['payslip_detail']['staff_id']);
		$data['employee'] = count($employee) > 0 ? $employee[0] : [];
		$data['list_department'] = $list_department;
	
		// Fetch timesheets data for the same year
		$timesheets_data = $this->get_timesheets_data($data['payslip_detail']['staff_id'], $year);
		...
		// Fetch staff details
		$staff_data = $this->get_staff_data($data['payslip_detail']['staff_id']);
		$data['staff_data'] = $staff_data;
	
		// Fetch job position details based on staff job_position field matching hr_job_position position_id
		$job_position_data = $this->get_job_position_data($staff_data['job_position']);
		$data['job_position_data'] = $job_position_data;
	...
	public function get_timesheets_data($staff_id, $year) {
		$this->db->where('staffid', $staff_id);
		$this->db->where('year', $year);
		$timesheets_data = $this->db->get(db_prefix() . 'timesheets_day_off')->result_array();
		return $timesheets_data;
	}
	
	// Function to fetch currency data with isdefault = 1
	...
	public function get_staff_data($staff_id) {
		$this->db->where('staffid', $staff_id);
		$staff_data = $this->db->get(db_prefix() . 'staff')->row_array();
		return $staff_data;
	}
	
	// Function to fetch job position data based on job_position field from staff table
	public function get_job_position_data($job_position) {
		$this->db->where('position_id', $job_position);
		$job_position_data = $this->db->get(db_prefix() . 'hr_job_position')->row_array();
	 ...
	 */
	 ...
	 public function payslip_manage_export_pdf($id)
	 {
		 ...
		 // Retrieve payslip and payslip details
		 ...
		 foreach (glob(HR_PAYROLL_EXPORT_EMPLOYEE_PAYSLIP . '*') as $file) {
			 $file_arr = explode("/", $file);
			 $filename = array_pop($file_arr);
			 if (file_exists($file) && $filename != 'index.html') {
		 ...
		 foreach ($payslip_details as $payslip_detail) {
			 ...
			 // Get staff data from tblstaff
			 $this->load->model('staff_model');
			 $staff = $this->staff_model->get($payslip_detail['staff_id']);
			 $data['staff'] = $staff;

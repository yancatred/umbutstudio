    ...
    <div class="horizontal-tabs">
        <ul class="nav nav-tabs nav-tabs-horizontal" role="tablist">
            ...
            <li role="presentation">
                <a href="#colors" aria-controls="colors" role="tab"
                    data-toggle="tab"><?php echo _l('settings_calendar_colors_heading'); ?></a>
            </li>
        </ul>
    </div>
</div>
<div class="tab-content mtop15">
    <div role="tabpanel" class="tab-pane active" id="general">
        ...
        <div class="form-group">
            ...
            <br />
            <select class="selectpicker" data-width="100%" name="settings[default_view_calendar]"
                id="default_view_calendar">
                <option value="dayGridMonth" <?php if (get_option('default_view_calendar') == 'dayGridMonth') {
    echo ' selected';
} ?>><?php echo _l('month'); ?></option>
                <option value="dayGridWeek" <?php if (get_option('default_view_calendar') == 'dayGridWeek') {
    echo ' selected';
} ?>><?php echo _l('week'); ?></option>
                <option value="dayGridDay" <?php if (get_option('default_view_calendar') == 'dayGridDay') {
    echo ' selected';
} ?>><?php echo _l('day'); ?></option>
                <option value="timeGridWeek" <?php if (get_option('default_view_calendar') == 'timeGridWeek') {
    echo ' selected';
} ?>><?php echo _l('agenda'); ?> <?php echo _l('week'); ?></option>
                <option value="timeGridDay" <?php if (get_option('default_view_calendar') == 'timeGridDay') {
    echo ' selected';
} ?>><?php echo _l('agenda'); ?> <?php echo _l('day'); ?></option>
            </select>
        </div>
        <hr />

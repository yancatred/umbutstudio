...
if (isset($member)) {
	...
	$title .= _l('ts_permission');
}
?>
				<h4 class="modal-title"><?php echo html_entity_decode($title); ?></h4>
			</div>
			<?php echo form_open(admin_url('timesheets/update_permissions/' . $staffid), array('id' => 'update_permissions')); ?>
...
?>
	...
	?>
						<div class="lable-display-name">
							<?php echo render_input('staff_name', 'staff_name', get_staff_full_name($member->staffid), '', $staff_attr); ?>
...
?>
					...
					<div class="class <?php echo html_entity_decode($display_staff); ?>">
						<?php echo render_select('staff_id', $staffs, array('staffid', array('firstname', 'lastname')), 'staff_name', $staff_selected); ?>
					</div>
...
$hr_profile_permissions = list_timesheet_permisstion();
